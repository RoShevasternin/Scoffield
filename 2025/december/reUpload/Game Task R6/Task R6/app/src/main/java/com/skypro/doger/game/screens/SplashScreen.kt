...
import com.skypro.doger.game.GDX_GLOBAL_isGame
import com.skypro.doger.game.actors.Koleso
import com.skypro.doger.game.game
import com.skypro.doger.game.manager.FontTTFManager
...
import com.skypro.doger.game.utils.advanced.AdvancedStage
import com.skypro.doger.game.utils.gdxGame
...
class SplashScreen : AdvancedScreen() {
    ...
    private fun loadingAssets() {
        if (isFinishLoading.not() && game.assetManager.update(17)) {
            ...
            coroutine.launch { mutex.withLock { while (progress < (game.assetManager.progress * 100)) {
                ...
                progress += 1
                if (progress == 100) isFinishLoading = true
    ...
    private fun navToMenu() {
        if (isFinishLoading && GDX_GLOBAL_isGame) {
