...
open class ACheckBox(
    ...
    open fun getListener() = object : InputListener() {
        ...
        override fun touchDown(event: InputEvent?, x: Float, y: Float, pointer: Int, button: Int): Boolean {
            screen.game.soundUtil.apply { play(punch, volumeLevel*0.25f) }
    ...
    fun getStyleByType(type: Static.Type) = when(type) {
        Static.Type.MUSIC   -> Static.ACheckBoxStyle(
            default = screen.game.allAssets.music_def,
            checked = screen.game.allAssets.music_press,
        )
        Static.Type.SOUND     -> Static.ACheckBoxStyle(
            default = screen.game.allAssets.sound_def,
            checked = screen.game.allAssets.sound_press,
        )
        Static.Type.PAUSE -> Static.ACheckBoxStyle(
            default = screen.game.allAssets.pause_def,
            checked = screen.game.allAssets.pause_press,
        )
    }
