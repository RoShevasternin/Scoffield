package com.fluxton.ballerina.game.actors.checkbox
...
import com.fluxton.ballerina.game.utils.TextureEmpty
...
open class ACheckBox(
    ...
    fun getStyleByType(type: Static.Type) = when(type) {
        Static.Type.PAUSE -> Static.ACheckBoxStyle(
            default = screen.game.allAssets.pus_def,
            checked = screen.game.allAssets.pus_check,
        )
        Static.Type.CHECK -> Static.ACheckBoxStyle(
            default = screen.game.allAssets.on,
            checked = screen.game.allAssets.off,
        )
    }
