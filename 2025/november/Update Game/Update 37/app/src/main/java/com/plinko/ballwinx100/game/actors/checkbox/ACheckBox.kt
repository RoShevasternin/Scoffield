package com.plinko.ballwinx100.game.actors.checkbox
...
open class ACheckBox(
    ...
    private val mainAssets by lazy { screen.game.mainAssets }

    override fun getHeight(): Float {
        return defaultImage.height
    }
    ...
    private fun asyncCollectCheckFlow() {
        coroutine?.launch {
            checkFlow.collect { isCheck ->
                if (isInvokeCheckBlock) {
                    onCheckBlock(isCheck)
                }  else {
                    Log.d("ACheckBox", "asyncCollectCheckFlow: oops")
                }
            }
        }
    }
    ...
    open fun getListener() = object : InputListener() {
        ...
        override fun touchDown(event: InputEvent?, x: Float, y: Float, pointer: Int, button: Int): Boolean {
            ...
            screen.game.soundUtil.apply { play(KLAK) }
            return true
        }
    ...
    fun getStyleByType(type: Static.Type) = when(type) {
        Static.Type.EFFECTS -> Static.ACheckBoxStyle(
            default = mainAssets.SWITCH_SOUND_ON,
            checked = mainAssets.SWITCH_SOUND_OFF,
        )
        Static.Type.MUSIC -> Static.ACheckBoxStyle(
            default = mainAssets.SWITCH_MUSIC_ON,
            checked = mainAssets.SWITCH_MUSIC_OFF,
        )
    }
