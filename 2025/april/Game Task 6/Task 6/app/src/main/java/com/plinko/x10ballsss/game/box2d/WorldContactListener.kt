package com.plinko.x10ballsss.game.box2d
...
class WorldContactListener: ContactListener {
    ...
    override fun beginContact(contact: Contact) {
        with(contact) {
            abstractBodyA.screenBox2d.game.soundUtil.apply { play(BIMS, 33f) }

//            playSound(abstractBodyA, abstractBodyB)
            abstractBodyA.beginContact(abstractBodyB, this)
            abstractBodyB.beginContact(abstractBodyA, this)
        }
    }

    override fun endContact(contact: Contact) {
        with(contact) {
            abstractBodyA.endContact(abstractBodyB, this)
            abstractBodyB.endContact(abstractBodyA, this)
        }
    }
...
//                -> if (currentTimeMinus(timeContactDynamic) >= 200) {
//                    soundUtil.apply { play(BOX, 15f) }
