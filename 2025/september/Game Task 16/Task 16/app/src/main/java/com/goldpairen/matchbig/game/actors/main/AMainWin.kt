...
//    )
    ...
    private val imgMenu = Image(gdxGame.assetsAll.VIKKKKKI)
    ...
    override fun addActorsOnGroup() {
        ...
        addActor(imgMenu)
        imgMenu.setBounds(115f, 418f, 850f, 1370f)

        addBtnS()

        val aX = Actor()
        addActor(aX)
        aX.setBounds(833f, 418f, 132f, 132f)
        aX.setOnClickListener(gdxGame.soundUtil) {
            screen.hideScreen {
    ...
    private fun addBtnS() {
        ...
        listBtn.forEachIndexed { index, btn ->
            addActor(btn)
            btn.setBounds(272f, ny, 537f, 149f)
            ny -= 68 + 149

            btn.setOnClickListener(gdxGame.soundUtil) {
                screen.hideScreen {
                    gdxGame.navigationManager.navigate(listNames[index])
                }
            }
        }
    }
