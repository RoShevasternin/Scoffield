...
class OlyFinishScreen(override val game: LibGDXGame) : AdvancedScreen() {

    override fun show() {
        stageUI.root.animHide()
        setUIBackground(if (IS_WIN) game.allAssets.OLYVERYNICE.region else game.allAssets.OLYVERYBAD.region)
        super.show()
        stageUI.root.animShow(TIME_ANIM_SCREEN_ALPHA)

        if (IS_WIN) game.soundUtil.apply { play(winner) }
        else game.soundUtil.apply { play(failuredrumsoundeffect) }
    }
