...
import com.hexofun.rundeo.game.GDX_GLOBAL_isGame
import com.hexofun.rundeo.game.GDX_GLOBAL_isLoadAssets
...
class HexagonLoaderScreen(override val game: LibGDXGame) : AdvancedScreen() {
    ...
    override fun show() {
        ...
        super.show()
        stageUI.addActor(Image(gdxGame.startAssets.lop).also {
            it.setBounds(767f, 347f, 385f, 385f)
            it.setOrigin(Align.center)
            it.addAction(Actions.forever(Actions.rotateBy(-360f, 1.75f)))
        })
    ...
    private fun loadSplashAssets() {
        with(game.spriteManager) {
            loadableTextureList = mutableListOf(
                ...
                SpriteManager.EnumTexture.lop.data,
            )
            loadTexture()
        }
