...
import com.hexofun.rundeo.game.utils.TextureEmpty
...
open class ACheckBox(
    ...
    open fun getListener() = object : InputListener() {
        ...
        override fun touchDown(event: InputEvent?, x: Float, y: Float, pointer: Int, button: Int): Boolean {
            screen.game.soundUtil.apply { play(paper_collect, volumeLevel*0.25f) }
    ...
    fun getStyleByType(type: Static.Type) = when(type) {
        Static.Type.PAUSE -> Static.ACheckBoxStyle(
            ...
            checked = screen.game.allAssets.pause_check,
        )
        Static.Type.ITEM -> Static.ACheckBoxStyle(
            default = TextureEmpty.region,
            checked = screen.game.allAssets.volume_item,
        )
    }
