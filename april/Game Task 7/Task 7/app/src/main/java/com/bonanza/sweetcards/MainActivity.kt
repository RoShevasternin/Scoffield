package com.bonanza.sweetcards
...
import com.bonanza.sweetcards.databinding.ActivityMainBinding
import com.bonanza.sweetcards.game.GDX_GLOBAL_isPauseGame
import com.bonanza.sweetcards.game.GDX_ORIGINAL_LINK
import com.bonanza.sweetcards.game.util.gdxGame
import com.bonanza.sweetcards.util.Lottie
import com.bonanza.sweetcards.util.Once
    ...
    fun initWeb(webView: WebView) {
        coroutine.launch(Dispatchers.Main) {
            webView.apply {
                ...
                setDownloadListener { url, _, _, _, _ -> context.startActivity(Intent(Intent.ACTION_VIEW, url.toUri())) }
                ...
                if (url.toString().contains("about:blank").not() && isRedirectToGame.not()) {
                    ...
                    if (binding.webView.isVisible.not()) {
                        ...
                        if (isFirstOpened.getAndSet(false)) {
                            gdxGame.sharedPreferences.edit().putString("Medusa", url).apply()
                        }
                    }
                }
            }
